...
def _plot_status(
    ...
    labels = ("Passed/completed", "Returned with warning", "Failed")
    ...
    ax.set_title(
        "Estimator status in 100 replications (pass / warning / failure)",
        pad=34,
    )
    fig.tight_layout()
    ...
    for facet_index, (ax, estimand) in enumerate(
        ...
        for i in range(len(SCENARIOS)):
            for j in range(len(PROCEDURES)):
                ...
                structural_zero = (
                    SCENARIOS[i] == "akm_dgp"
                    and PROCEDURES[j] == "project_plugin_bic"
    ...
    *,
    ...
    _write_latex(
        ...
        notes=(
            ...
            "warning / failed attempts out of 100."
        ),
    )
...
def _write_markdown(
    ...
    *,
    ...
    lines = [
        ...
        *status_table,
        "",
        f"![Figure 1. Pass or completion, returned-with-warning, and failed attempts. KSS and BS20 use completion only.]({relative('status-matrix.png')})",
        "",
        ...
        "",
        "The two `0*` cells require a different explanation. Under the AKM DGP, the project truth has $h_{ij}=0$, hence $H_F=\\rho_H=0$. BIC also selects rank zero in all 100 AKM replications, so the fitted project model imposes the same zeros. Their raw RMSEs are "
        ...
        "",
        f"![Figure 2. RMSE against the common project truth among all returned estimates. N/A means not reported; 0* marks AKM structural zeros.]({relative('common-target-rmse.png')})",
        "",
        ...
        *gap_table,
        "",
        f"![Figure 3. KSS and BS20 native population targets minus the corresponding project targets.]({relative('native-project-target-gaps.png')})",
        "",
